...
/// managed hot-unplug, by turning the slot's Power Indicator off.
///
/// The acknowledgement arrives as a config write on a vCPU thread, but tearing
/// the device down needs the VMM thread, which owns the event manager. So the
/// port only records its secondary bus here and signals `evt`; the VMM drains
/// `acked_buses` when `evt` fires.
#[derive(Debug)]
pub struct HotplugCompletion {
    /// Signalled when at least one slot has acknowledged removal.
    pub evt: EventFd,
    /// Secondary buses whose endpoints are ready to be torn down.
    pub acked_buses: Mutex<Vec<u8>>,
}
...
pub struct PciRootPort {
    ...
    link_status: u16,
    secondary_bus: u8,
    /// Whether a managed removal is waiting for the guest to power the slot
...
impl PciRootPort {
    ...
    /// * `completion` - channel for reporting acknowledged managed removals
    pub fn new(
        sbdf: PciSBDF,
        secondary_bus: u8,
    ...
    /// port as an inclusive `(base, limit)` pair, or `None` if the window is
    /// disabled.
    pub fn nonpref_memory_window(&self) -> Option<(u64, u64)> {
...
impl PciDevice for PciRootPort {
    fn write_config_register(
        ...
        if in_bars {
            ...
            if self.write_slot_dword(offset, data) && self.removal_requested {
                // The guest has quiesced the device and powered the slot off,
                // so the backend can go. Hand the slot to the VMM thread.
                self.removal_requested = false;
                self.completion
                    .acked_buses
                    .lock()
                    .expect("Poisoned lock")
                    .push(self.secondary_bus);
...
mod tests {
    ...
    #[test]
    fn test_managed_removal_reports_the_guest_ack() {
        ...
        assert!(completion.acked_buses.lock().unwrap().is_empty());
        ...
        rp.write_config_register(reg, 0, &PCI_EXP_SLTCTL_PWR_IND_OFF.to_le_bytes());
        assert_eq!(*completion.acked_buses.lock().unwrap(), vec![7]);
